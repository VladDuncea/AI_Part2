...
class Joc:
	...
	def final(self):
		...
		rez = (elem_identice(self.matr[0:3])			# l1
				or elem_identice(self.matr[3:6])		# l2
				or elem_identice(self.matr[6:9])		# l3
				or elem_identice(self.matr[0:9:3])		# c1
				or elem_identice(self.matr[1:9:3])		# c2
				or elem_identice(self.matr[2:9:3])		# c3
				or elem_identice(self.matr[0:9:4])		# d1
				or elem_identice(self.matr[2:8:2]))		# d2
		if rez:
			return rez
	...
	def mutari_joc(self, jucator):
		...
		for i in range(len(self.matr)):
			# cautam locurile goale
			if self.matr[i] == Joc.GOL:
				# ocupam pozitia goala
				tabla_noua = self.matr.copy()
				tabla_noua[i] = jucator
	...
	def linii_deschise(self, jucator):
		return (self.linie_deschisa(self.matr[0:3], jucator)
				...
				+ self.linie_deschisa(self.matr[0:9:4], jucator)  	# d1
				+ self.linie_deschisa(self.matr[2:8:2], jucator))  	# d2
